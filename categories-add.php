<?php
    ...
    if(isset($_POST['btnSave'])) {
        ...
        $sqlCheck = "SELECT cat_name FROM categories_tbl WHERE cat_name = '$txtName'";
        $queryCheck = mysqli_query($dbConString, $sqlCheck);

        if(mysqli_num_rows($queryCheck) > 0){
            $message = "Category is already exists. Please try enter another category.";
            echo "<script type='text/javascript'>alert('$message');</script>";
        }else{
            $sqlAdd = "INSERT INTO categories_tbl() VALUES (NULL, '$txtName', '$txtDescription', '$date', 0)";
